...
const PLATFORMS = [
  { id: "tiktok", name: "TikTok", icon: "\u266B", color: "#1A1A1A", linked: true },
  { id: "linkedin", name: "LinkedIn", icon: "in", color: "#0A66C2", linked: true },
  { id: "youtube", name: "YouTube", icon: "\u25B6", color: "#FF0000", linked: true },
  { id: "instagram", name: "Instagram", icon: "\uD83D\uDCF7", color: "#E4405F", linked: false },
  { id: "gmb", name: "Google Business", icon: "G", color: "#4285F4", linked: true },
]
      ...
      <div style={{ minHeight: "100vh", backgroundColor: "#F4F1E4", padding: "36px 24px" }}>
        <div style={{ maxWidth: 660, margin: "0 auto" }}>
          ...
          <div style={{ fontFamily: BODY, fontStyle: "italic", fontSize: 13, color: "#8B7355", marginBottom: 24 }}>
            Connect your accounts to auto-build your Content DNA from real post history
          </div>

          {/* Platform grid */}
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 20 }}>
            {PLATFORMS.map(p => (
              <div key={p.id} style={{ backgroundColor: "#FDFCF8", border: `1px solid ${p.linked ? "#3E2723" : "#C4B9A0"}`, padding: "14px 18px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                <div>
                  <div style={{ fontFamily: DISPLAY, fontWeight: 700, fontSize: 14, color: "#1A1A1A" }}>{p.name}</div>
                  <div style={{ fontFamily: "system-ui", fontSize: 10, fontWeight: 700, textTransform: "uppercase", color: p.linked ? "#065F46" : "#8B7355", marginTop: 2 }}>
                    {p.linked ? "\u2713 Connected" : "Not connected"}
                  </div>
                </div>
                <div style={{ width: 34, height: 34, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "system-ui", fontWeight: 700, fontSize: 16, color: p.linked ? p.color : "#C4B9A0", opacity: p.linked ? 1 : 0.4 }}>
                  {p.icon}
                </div>
              </div>
            ))}
          ...
          <div style={{ fontFamily: TYPEWRITER, fontSize: 10, color: "#8B7355", textAlign: "center", marginTop: 5 }}>
            Pulls your last 10 posts from each linked platform and analyses them with AI
          </div>
