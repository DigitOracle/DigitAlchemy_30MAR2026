...
- [ ] **2.0.8** Security fix: make /api/content-dna/auto-ingest auth mandatory (currently optional). Investigate /api/jobs/[jobId]/stream and /api/reverse-engineer auth needs.
...
- **v1.3 — April 7, 2026 (Session 5)** — Phase 2.3.7 complete — second security fix for /api/post-recommendations, /api/dashboard, /api/content-dna/analyze. All three routes now require Firebase Auth. Sweep of remaining app/api/ routes found 1 additional gap (/api/content-dna/auto-ingest has optional auth) plus 2 routes to investigate (jobs/stream, reverse-engineer). Deferred to Phase 2.0.8.
